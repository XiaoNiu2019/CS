...
msg_box msg_box_init(const unsigned elt_size) {
    ...
    CHECK_NZ(pthread_mutex_init(&(mbox->mutex) , NULL));
    CHECK_NZ(pthread_cond_init(&(mbox->not_empty) , NULL));
    return mbox;
}
...
int msg_box_receive(msg_box mbox, char *buf) {
  ...
  while ((*mbox).empty)
  /* Wait until being triggered by the condition variable*/
...
int msg_box_send(msg_box mbox, const char *buf) {
  ...
  (*mbox).empty=false;
  /* Notify the other thread about the code modification */
